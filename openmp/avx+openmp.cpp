...
float Distance(const node& X, const node& Z, long long n) {
    float result = 0;
    #pragma omp parallel for num_threads(4) reduction(+:result) schedule(guided,256)
    for (long long i = 0; i < n; i += 8) {
        __m256 x = _mm256_loadu_ps(&X.dimen[i]);
        __m256 z = _mm256_loadu_ps(&Z.dimen[i]);
        __m256 diff = _mm256_sub_ps(x, z);
        __m256 diff_sq = _mm256_mul_ps(diff, diff);
        result += _mm256_cvtss_f32(_mm256_hadd_ps(diff_sq, diff_sq));
    }
    return sqrt(result);
}
...
void Add(node& result, const node& X, long long n) {
    ...
    for (long long i = 0; i < n; i += 8) {
        __m256 r = _mm256_loadu_ps(&result.dimen[i]);
        __m256 x = _mm256_loadu_ps(&X.dimen[i]);
        __m256 sum = _mm256_add_ps(r, x);
        _mm256_storeu_ps(&result.dimen[i], sum);
    }
}
...
void Kmeans(long long k, vector<node>& data, long long n, long long m) {
    ...
    while (cluster_changed) {
        ...
        #pragma omp parallel for num_threads(4) schedule(guided,256) reduction(+:new_centers[:k*m], counts[:k])
        for (long long i = 0; i < n; i++) {
            int cluster = idx[i];
