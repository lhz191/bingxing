...
float Distance(const node& X, const node& Z, long long n) {
    float result = 0;
    #pragma omp parallel for reduction(+:result) num_threads(4) schedule(guided,256)
    for (long long i = 0; i < n; i += 4) {
        float32x4_t vecX = vld1q_f32(&X.dimen[i]);
        float32x4_t vecZ = vld1q_f32(&Z.dimen[i]);
        float32x4_t diff = vsubq_f32(vecX, vecZ);
        float32x4_t sqr = vmulq_f32(diff, diff);
        result += vaddvq_f32(sqr);  // Sum up all elements in the vector
    }
    return sqrt(result);
}
...
void Add(node& result, const node& X, long long n) {
    ...
    for (long long i = 0; i < n; i += 4) {
        float32x4_t vecResult = vld1q_f32(&result.dimen[i]);
        float32x4_t vecX = vld1q_f32(&X.dimen[i]);
        vecResult = vaddq_f32(vecResult, vecX);
        vst1q_f32(&result.dimen[i], vecResult);
    }
}
...
void Kmeans(long long k, vector<node>& data, long long n, long long m) {
    ...
    while (cluster_changed) {
        ...
        // 2.2 更新簇中心
        vector<vector<float>> new_centers(k, vector<float>(m, 0.0f));
        ...
        for (long long i = 0; i < n; i++) {
            ...
            for (long long j = 0; j < m; j++) {
                #pragma omp atomic
                new_centers[cluster][j] += data[i].dimen[j];
            }
            #pragma omp atomic
            counts[cluster]++;
        }
        ...
        for (long long i = 0; i < k; i++) {
            if (counts[i] > 0) {
                for (long long j = 0; j < m; j++) {
                    C[i].dimen[j] = new_centers[i][j] / counts[i];
                }
            }
        }
