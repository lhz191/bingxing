...
void Kmeans(long long k, vector<node>& data, long long n, long long m) {
    ...
    while (cluster_changed) {
        ...
        // Reduce centroids and cluster sizes across all processes
        vector<float> global_centroids(k * m, 0);
        vector<int> global_clusterSizes(k, 0);
        MPI_Allreduce(h_centroids.data(), global_centroids.data(), k * m, MPI_FLOAT, MPI_SUM, MPI_COMM_WORLD);
        MPI_Allreduce(h_clusterSizes.data(), global_clusterSizes.data(), k, MPI_INT, MPI_SUM, MPI_COMM_WORLD);
...
int main(int argc, char** argv) {
    ...
    omp_set_num_threads(4);  // Set the number of OpenMP threads to 4
