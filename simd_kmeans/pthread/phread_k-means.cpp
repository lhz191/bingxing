...
void* compute_distance(void* arg) {
    ...
    for (long long i = args->start; i < args->end; i++) {
        result += (args->X[i] - args->Z[i]) * (args->X[i] - args->Z[i]);
    }
    args->result = result;
    pthread_exit(NULL);
}

float Distance(const float* X, const float* Z, long long n) {
    ...
    for (int i = 0; i < NUM_THREADS; i++) {
        ...
        args[i].end = (i == NUM_THREADS - 1) ? n : (i + 1) * block_size;
        pthread_create(&threads[i], NULL, compute_distance, &args[i]);
    }

    for (int i = 0; i < NUM_THREADS; i++) {
        pthread_join(threads[i], NULL);
        total_result += args[i].result;
    }
...
float Distance(const node& X, const node& Z, long long n) {
    return Distance(X.dimen.data(), Z.dimen.data(), n);
}
