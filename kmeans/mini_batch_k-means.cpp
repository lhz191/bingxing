...
void VectorizedUpdateClusterCenter(const std::vector<Node>& data, const std::vector<int>& idx, int k, int n, int m) {
    for (int j = 0; j < k; j++) {
        ...
        for (int i = 0; i < n; i += 4) {
            // 将整型簇索引转换为浮点型
            __m128 mask = _mm_cmpeq_ps(_mm_loadu_ps(reinterpret_cast<const float*>(&idx[i])), _mm_set1_ps(static_cast<float>(j)));
            __m128 data_vec = _mm_and_ps(_mm_loadu_ps(&data[i].dimensions[0]), mask);
            sum = _mm_add_ps(sum, data_vec);
            count += _mm_movemask_ps(mask);
        }

        if (count > 0) {
            __m128 center = _mm_div_ps(sum, _mm_set1_ps(static_cast<float>(count)));
            _mm_storeu_ps(const_cast<float*>(&data[j].dimensions[0]), center);
        }
    }
}
...
void MiniBatchKMeans(int k, std::vector<Node>& data, int n, int m, int batchSize) {
    ...
    while (cluster_changed) {
        ...
        for (int batch_idx = 0; batch_idx < num_batches; batch_idx++) {
            ...
            // 更新簇中心
            VectorizedUpdateClusterCenter(data, idx, k, batch_size, m);
        }
    }
