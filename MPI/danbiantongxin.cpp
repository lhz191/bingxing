...
void Kmeans(long long k, vector<node>& data, long long n, long long m) {
    ...
    // 2. 广播聚类中心到其他进程（使用单边通信）
    MPI_Win win;
    MPI_Win_create(C[0].dimen.data(), k * m * sizeof(float), sizeof(float), MPI_INFO_NULL, MPI_COMM_WORLD, &win);
    MPI_Win_fence(0, win);
    if (rank != 0) {
        MPI_Get(C[0].dimen.data(), k * m, MPI_FLOAT, 0, 0, k * m, MPI_FLOAT, win);
    }
    MPI_Win_fence(0, win);
    ...
    while (cluster_changed) {
        ...
        // 使用单边通信进行全局归约
        MPI_Win local_win, count_win;
        MPI_Win_create(local_centers[0].dimen.data(), k * m * sizeof(float), sizeof(float), MPI_INFO_NULL, MPI_COMM_WORLD, &local_win);
        MPI_Win_create(local_counts.data(), k * sizeof(long long), sizeof(long long), MPI_INFO_NULL, MPI_COMM_WORLD, &count_win);

        MPI_Win_fence(0, local_win);
        MPI_Win_fence(0, count_win);

        for (long long j = 0; j < k; ++j) {
            MPI_Accumulate(local_centers[j].dimen.data(), m, MPI_FLOAT, 0, j * m, m, MPI_FLOAT, MPI_SUM, local_win);
            MPI_Accumulate(&local_counts[j], 1, MPI_LONG_LONG, 0, j, 1, MPI_LONG_LONG, MPI_SUM, count_win);
        }
