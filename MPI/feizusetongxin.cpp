...
void Kmeans(long long k, vector<node>& data, long long n, long long m) {
    ...
    // 2. 广播聚类中心到其他进程
    vector<MPI_Request> requests(k);
    for (long long i = 0; i < k; ++i) {
        MPI_Ibcast(C[i].dimen.data(), m, MPI_FLOAT, 0, MPI_COMM_WORLD, &requests[i]);
    }
    ...
    while (cluster_changed) {
        ...
        vector<MPI_Request> requests_centers(k);
        vector<MPI_Request> requests_counts(k);
        for (long long j = 0; j < k; ++j) {
            MPI_Iallreduce(local_centers[j].dimen.data(), global_centers[j].dimen.data(), m, MPI_FLOAT, MPI_SUM, MPI_COMM_WORLD, &requests_centers[j]);
            MPI_Iallreduce(&local_counts[j], &global_counts[j], 1, MPI_LONG_LONG, MPI_SUM, MPI_COMM_WORLD, &requests_counts[j]);
        }
        MPI_Waitall(k, requests_centers.data(), MPI_STATUSES_IGNORE);
        MPI_Waitall(k, requests_counts.data(), MPI_STATUSES_IGNORE);
        ...
        MPI_Iallreduce(&local_cluster_changed, &global_cluster_changed, 1, MPI_INT, MPI_LOR, MPI_COMM_WORLD, &requests[0]);
