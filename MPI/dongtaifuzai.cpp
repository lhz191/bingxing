...
void Kmeans(long long k, vector<node>& data, long long n, long long m) {
    ...
    while (cluster_changed) {
        ...
        if (rank == 0) {
            // 主进程负责管理任务队列
            vector<int> task_queue(total_chunks);
            iota(task_queue.begin(), task_queue.end(), 0); // 初始化任务队列
            int completed_chunks = 0;
            while (completed_chunks < total_chunks) {
                MPI_Status status;
                int worker_rank;
                MPI_Recv(&worker_rank, 1, MPI_INT, MPI_ANY_SOURCE, 0, MPI_COMM_WORLD, &status); // 等待工作进程请求任务
                if (!task_queue.empty()) {
                    int task = task_queue.back();
                    task_queue.pop_back();
                    MPI_Send(&task, 1, MPI_INT, worker_rank, 1, MPI_COMM_WORLD); // 分配任务
                } else {
                    int task = -1; // 没有更多任务，发送结束信号
                    MPI_Send(&task, 1, MPI_INT, worker_rank, 1, MPI_COMM_WORLD);
                    completed_chunks++;
                }
            }
        } else {
            ...
            while (true) {
                int flag = rank;
                MPI_Send(&flag, 1, MPI_INT, 0, 0, MPI_COMM_WORLD); // 请求任务
                int task;
                MPI_Recv(&task, 1, MPI_INT, 0, 1, MPI_COMM_WORLD, MPI_STATUS_IGNORE); // 接收任务
                if (task == -1) break; // 收到结束信号，退出循环
                long long start = task * chunk_size;
        ...
        for (long long i = 0; i < n; ++i) {
            Add(local_centers[idx[i]], data[i], m);
            local_counts[idx[i]]++;
        }
