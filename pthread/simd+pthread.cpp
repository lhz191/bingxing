...
float Distance(const node& X, const node& Z, long long m) {
    ...
    float result = 0.0f;
    __m256 hsum = _mm256_hadd_ps(_mm256_hadd_ps(sum, sum), sum);
...
void Add(node& result, const node& X, long long n) {
    ...
    for (; i < n - (n % 8); i += 8) {
        // 使用 AVX2 指令加载数据
        __m256 result_vec = _mm256_loadu_ps(&result.dimen[i]);
        __m256 x_vec = _mm256_loadu_ps(&X.dimen[i]);

        // 执行向量加法
        result_vec = _mm256_add_ps(result_vec, x_vec);

        // 将结果存储回内存
        _mm256_storeu_ps(&result.dimen[i], result_vec);
    }
