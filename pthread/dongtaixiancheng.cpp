...
void worker_thread(vector<node>& data, vector<node>& C, vector<long int>& idx, long long i, long long k, long long m) {
    ...
    while (cluster_changed) {
        ...
        vector<threadParam_t> params(NUM_THREADS);
            ...
            for (int t_id = 0; t_id < NUM_THREADS && i + t_id < n; ++t_id) {
                ...
                params[t_id].k = k;
                params[t_id].m = m;
                pthread_create(&worker_threads[t_id], NULL, worker_thread, &params[t_id]);
            }
        ...
        // 2.2 更新簇中心
        for (long long j = 0; j < k; ++j) {
            const int UPDATE_THREADS = 4; // 更新簇中心的线程数
            vector<pthread_t> update_threads(UPDATE_THREADS);
            vector<updateParam_t> update_params(UPDATE_THREADS);

            for (long long i = 0; i < n; i += UPDATE_THREADS) {
                // 创建线程并执行更新簇中心的任务
                for (int t_id = 0; t_id < UPDATE_THREADS && i + t_id < n; ++t_id) {
                    ...
                    pthread_create(&update_threads[t_id], NULL, update_thread, &update_params[t_id]);
                }
